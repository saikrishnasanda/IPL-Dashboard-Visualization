...
def all_palyer_info(df_mat, df_bb, all_players) : 
    ...
    with st.container () : 
        ...
        with txt3 : 
            st.write(f"##### Total Matches Loose : {sig_play.shape[0] - no_win_match}")
    ...
    if is_batter.shape[0] != 0: 
        ...
        for itm in All_seasons : 
            ...
            ax_obj = sns.barplot(x=['Total Runs', 'Total Bowls', 'Total 6th', 'Total 4th'], y=[itm_runs, itm_balls, itm_6th, itm_4th], ax=itm_ax)
    ...
    if is_baller.shape[0] != 0 :
        ...
        for itm in All_seasons : 
            ...
            itm_fig, itm_ax = plt.subplots()
            itm_ax.set_title(f"In {itm} Season {curr_player} Performance")
            ax_obj = sns.barplot(x=['Total Wickets', 'Total Bowls', 'Total Runs', 'Total Meaiden'], y=[itm_wic, itm_balls, itm_runs, is_meaiden[is_meaiden['total_run'] == 0].shape[0]], ax=itm_ax)
